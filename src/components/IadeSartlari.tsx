...
interface IadeSartlariProps {
    embedded?: boolean;
    mode?: 'all' | 'delivery' | 'returns' | 'compact';
}

export default function IadeSartlari({ embedded = false, mode = 'all' }: IadeSartlariProps) {
    ...
    const getList = (key: string): string[] => {
        try {
            ...
            if (Array.isArray(rawValue)) {
                ...
                return Object.values(rawValue) as string[];
            }
            return [];
        } catch (error) {
            return [];
    ...
    return (
        <div className={`${classes.container} ${embedded ? classes.embedded : ''} ${embedded ? classes.simpleLayout : ''}`}>
            {!embedded && (
                <>
                    <h1>{t('title')}</h1>
            ...
            {/* Compact summary mode for ProductDetailCard */}
            {mode === 'compact' && (
                <div className={classes.compactSummary}>
                    <p style={{ whiteSpace: 'pre-line' }}>{t('summary')}</p>
                    <Link href={`/${locale}/iade-sartlari`} className={classes.readMoreLink}>
                        {t('readMore')} →
                    </Link>
                </div>
            )}
            ...
            {(mode === 'all' || mode === 'delivery' || mode === 'returns') && (
                <>
                    <section className={classes.section}>
                        <h2>{t('delivery.title')}</h2>
                        <div className={classes.deliveryBox}>
                            <p className={classes.deliveryTime}>{t('delivery.time')}</p>
                            <p>{t('delivery.text')}</p>
                        </div>
                    </section>
                </>
