...
/**
 * Visual editor overlay. Only renders when the page is loaded with
 * `?edit=1` query (i.e. the ERP storefront preview iframe). Hovers any
 * element marked with `data-edit-zone="<kind>"` highlights it with a
 * teal outline + "Düzenle" badge. Clicking dispatches a postMessage
 * to the parent ERP, which jumps to that section's edit form.
 *
 * Future hooks (kept lightweight here, expanded in later phases):
 *   - data-edit-sort-list  → enable drag-drop reorder
 *   - data-edit-inline     → contentEditable inline text edit
 */
...
export default function EditorOverlay() {
  ...
  useEffect(() => {
    ...
    // Hidden file input reused by all image-edit clicks.
    ...
    fileInput.addEventListener('change', async () => {
      ...
      // Encode as base64 and ship via postMessage so the ERP parent
      // (same-origin with the API) handles the upload. Avoids
      // cross-origin cookie / SameSite headaches when the iframe POSTs
      // straight to ERP.
    ...
    function commitText(el: HTMLElement) {
      ...
      if (!model || !pk || !field) return;
      window.parent.postMessage(
        { source: 'demfirat-editor', type: 'text', model, pk, field, value },
        '*',
      );
    }

    function onClick(e: MouseEvent) {
      // Image edit click → file picker.
      ...
      // Text edit click → contentEditable.
      ...
      // Sortable items don't trigger zone-select on click — they're dragged.
      ...
      const zone = el.dataset.editZone || '';
      window.parent.postMessage(
        { source: 'demfirat-editor', type: 'select', zone },
        '*',
      );
    }
    ...
    // In edit mode the page is LOCKED — clicking nav/cards/buttons
    // must not navigate away. Otherwise the user accidentally leaves
    // their work-in-progress page just by missing an editable target.
    // The ERP picks the preview URL via its own page selector.
    function blockNavigation(e: MouseEvent) {
      // Modifier-click → open in new tab is fine, leave it alone.
      if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey || e.button !== 0) return;
      const link = (e.target as Element)?.closest?.('a[href]') as HTMLAnchorElement | null;
      if (!link) return;
      const href = link.getAttribute('href') || '';
      if (!href || href.startsWith('#')) return;
      // Block ALL link navigation in edit mode. Editable elements
      // (data-edit-text / image / sort-id / zone) are handled by their
      // own dedicated click handlers above.
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    document.addEventListener('click', onClick, true); // capture so original handlers don't fire
    document.addEventListener('mouseleave', onLeave);

    // Lazy-load SortableJS, then enable sorting on every list with
    // data-edit-sort-list. On drop, send {model, items:[{id,order}]}
    // to the parent ERP, which calls the reorder API + reloads.
    ...
    // Load SortableJS once from CDN if not already on the page.
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .sf-edit-mode [data-edit-sort-list].sf-edit-sorting [data-edit-sort-id] { transition: transform 220ms cubic-bezier(.22,1,.36,1); }
        .sf-edit-ghost {
        ...
        /* Floating handle for full-bleed images that sit under content
           (e.g. hero background). Hidden outside edit mode. */
